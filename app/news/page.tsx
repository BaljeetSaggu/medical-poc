...
import { CalendarDays, Heart, MessageCircle, Search, User } from "lucide-react";
...
export const metadata: Metadata = {
  title: "News",
  description:
    "Browse Meddical blog posts, recent articles, and health-related updates.",
};
  ...
  return (
    <div className="bg-white">
      ...
      <section className="relative h-[220px] w-full overflow-hidden sm:h-[260px]">
        ...
        <Image src="/images/blog.png" alt="Blog posts banner" fill className="object-cover" />
      ...
      <main id="main-content" className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-3">
          <div className="space-y-10 lg:col-span-2">
            {newsPosts.map((item) => (
              <article key={item.slug} className="overflow-hidden rounded-lg bg-white shadow-md">
                <Image
                  src={item.image}
                  alt={item.title}
                  width={900}
                ...
                <div className="p-8">
                  ...
                  <h2 className="mb-4 text-3xl font-bold text-blue-900">{item.title}</h2>
                  <p className="mb-6 leading-relaxed text-gray-600">{item.description}</p>

                  <Link
                    href={`/news/${item.slug}`}
                    className="inline-flex rounded-full bg-blue-900 px-6 py-3 text-white transition hover:bg-blue-700"
                  >
                    Read More
                  </Link>
                </div>
